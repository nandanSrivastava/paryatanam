      ...
      <div className="bg-white rounded-3xl p-8 md:p-10 shadow-luxury border border-gray-100 hover:shadow-2xl transition-shadow duration-500">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div key={day.day} className="mb-4">
                ...
                <ul className="space-y-1">
                  {day.inclusions.map((item) => (
                    <li
                      key={item}
                      className="flex items-start gap-3 text-base text-gray-700 group hover:bg-primary/5 p-1 rounded-xl transition-all"
                    >
            ...
            {!itinerary?.length && (
              <ul className="space-y-2">
                {inclusions.map((item, index) => (
                  <li
                    key={index}
                    className="flex items-start gap-3 text-base text-gray-700 group hover:bg-primary/5 p-2 rounded-xl transition-all"
                  >
                    <Check className="w-5 h-5 text-primary shrink-0 mt-0.5 group-hover:scale-125 transition-transform" />
                    <span className="leading-relaxed">{item}</span>
                  </li>
          ...
          <div>
              ...
              {stayDetails?.length ? (
                <ul className="space-y-2">
                  {stayDetails.map((item) => (
                    <li
                      key={item}
                      className="flex items-start gap-3 text-base text-gray-700 hover:bg-neutral-50 p-1 rounded-xl transition-all"
                    >
            ...
            <div>
              ...
              <ul className="space-y-2">
                {exclusions.map((item) => (
                  <li
                    key={item}
                    className="flex items-start gap-3 text-base text-gray-700 hover:bg-red-50 p-2 rounded-xl transition-all"
                  >
            ...
            {importantNote && (
              <div className="mt-4 rounded-2xl border-2 border-amber-200 bg-gradient-to-br from-amber-50 to-orange-50 p-4 text-sm text-amber-900 flex items-start gap-3 shadow-sm">
          ...
          <div className="bg-white rounded-3xl p-8 md:p-10 shadow-luxury border border-gray-100 hover:shadow-2xl transition-shadow duration-500">
            ...
            <ul className="space-y-3">
              {knowBeforeYouGo?.map((item) => (
                <li
                  key={item}
                  className="flex items-start gap-3 text-base text-gray-700 hover:bg-secondary/10 rounded-xl transition-all group"
                >
